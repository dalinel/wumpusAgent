...
#                 agent.holding.append(things[0])
    ...
    def random_location_inbounds_wumpus(self, exclude=None):
        """Returns a random location that is inbounds (within walls if we have walls)"""
...
class WumpusEnvironment(XYEnvironment):
    #pit_probability : Probability to spawn a pit in a location. (From Chapter 7.2)
    # Room should be 4x4 grid of rooms. The extra 2 for walls
    
    def __init__(self, agent_program, width=6, height=6,pit_probability=0.05):
    ...
    def init_world(self, program):
        ...
        "WUMPUS"
        w_x, w_y = self.random_location_inbounds_wumpus(exclude=(1, 1))
        ...
        "GOLD"
        lax, lay = self.random_location_inbounds_wumpus(exclude=(1, 1))
        location = (lax, lay)
    ...
    def percepts_from(self, agent, location, tclass=Thing):
        ...
        result = [thing_percepts.get(thing.__class__, thing) for thing in self.things
                  if thing.location == location and isinstance(thing, tclass) and not isinstance(thing,Explorer)]
    ...
    def percept(self, agent):
        ...
        #result.append(self.percepts_from(agent, (x, y + 1)))
        return self.percepts_from(agent, (x, y))
                ...
                if (isinstance(agent,Explorer)):
                    if agent.alive:
                        ...
                        for act in acts[:]:
                            ...
                            self.execute_action(agent,act)
                            print(self.percept(agent))
                            percepts_result = self.percept(agent)

            self.exogenous_change()
        return percepts_result
